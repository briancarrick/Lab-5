...
// (RED_LED)PA.5 bit band address
#define RED_LED  ((volatile unsigned char *) 0x42087F94)
// (GREEN_LED)PA.6 bit band address
#define GREEN_LED  ((volatile unsigned char *) 0x42087F98)
// (YELLOW_LED)PA.7 bit band address
...
void makeOpenBox(unsigned short x,unsigned short y,unsigned short rgb) //create outline of box @x,y of color given by rgb
{
	...
	{		
		...
		for(boxCol = 0; boxCol < 80; boxCol++){
			if(boxRow<5 || boxRow>75){
				writeDat(rgb); // write edge of box pixels to rgb color
			}
			else if(boxCol<5 || boxCol>75){
				writeDat(rgb);	// write edge of box pixels to rgb color				
			}
			else{
				writeDat(black); // write inside of box pixels to black color			
			}
		}
